...
class Simulations:
    ...
    def __call__(self, params, simulation_type='PWF', iseed=1234, sigma_t=5e-9, number_of_events=1):
        ...
        if (self.simulation_type == 'PWF'):
            tants[0, :] = wf.PWF_simulation(params, self.antennas.coordinates, iseed=iseed, sigma_t=sigma_t)
            for i in range(1, number_of_events):
                tants[i, :] = wf.PWF_simulation(params, self.antennas.coordinates, sigma_t=sigma_t)
        elif (self.simulation_type == 'SWF'):
            tants[0, :] = wf.SWF_simulation(params, self.antennas.coordinates, iseed=iseed, sigma_t=sigma_t)
            for i in range(1, number_of_events):
                tants[i, :] = wf.SWF_simulation(params, self.antennas.coordinates, sigma_t=sigma_t)
