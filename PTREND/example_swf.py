...
arr = np.load('/Users/mguelfan/Documents/GRAND/ADF_DC2/output_recons_DC2/test_aurelien/9596.npy')
with open('/Users/mguelfan/Documents/GRAND/ADF_DC2/output_recons_DC2/test_aurelien/9596.json', 'r') as f:
    event_params = json.load(f)

ev_du_pos = arr[:, 1:4]
tmax_3d = arr[:, 4:7] * 1e-9 + np.random.randn(*(arr[:, 4:7].shape)) * sigma_timing
#tmax_3d = arr[:, 4:7] * 1e-9 
Emax_3d = arr[:, 7:10]
...
print('theta_pwf', theta_pwf_rad*pwf.phys_params.R2D)
...
swf_fit = swf.get_SWF_fit(x_ants, t_ants, initial_guess,ncall=500)
