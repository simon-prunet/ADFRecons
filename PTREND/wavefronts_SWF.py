...
def SWF_simulation_xyz(Xants, x_eff, y_eff, z_eff, t_s):
    tants = SWF_model_xyz(Xants, x_eff, y_eff, z_eff, t_s)
    #n = np.random.standard_normal(tants.size) * sigma_t
    n = np.random.standard_normal(tants.size) 
    return (tants + n)


#def get_SWF_fit(x_ants, t_ants, initial_guess, ncall=800):
def get_SWF_fit(x_ants, t_ants, initial_guess, sigma_t=5e-9, ncall=800):

    x = x_ants[:, 0]
    y = x_ants[:, 1]
    z = x_ants[:, 2]

    #leastsquares = cost.LeastSquares((x, y, z), t_ants, t_ants*0, SWF_model_xyz)
