...
def compute_delay(omega,Xmax,Xb,U,K,alpha,delta, xmaxDist):
    ...
    X = compute_observer_position(omega,Xmax,U,K,xmaxDist,alpha)
...
def PWF_chi2(zenith, azimuth, Xants, tants, nants, sigma=5e-9) :
    K = np.array([np.sin(zenith) * np.cos(azimuth), np.sin(zenith) * np.sin(azimuth), np.cos(zenith)])
    #event_coord_shower_axis = np.sum(K.T * Xants, axis=1) #dot product
    event_coord_shower_axis = np.dot(Xants, K)
    time_delay_rec = event_coord_shower_axis/c_light 
    #print('time delay rec', time_delay_rec)

    delta_exp = tants - tants.min()
    #print('tants min', tants.min())
    delta_rec_plane =  time_delay_rec - time_delay_rec.min()
    chi2_plane = np.sum((delta_exp - delta_rec_plane)**2/(sigma**2*(nants-2))) #2 degrees of freedom
    #print('chi2 plan', chi2_plane)
    return chi2_plane

def SWF_chi2(Xmax, Xants, tants, nants, sigma=5e-9) :
    time_delay_rec_sphere = np.sqrt(np.sum((Xants - Xmax)**2, axis=1)) / c_light 
    delta_exp = tants - tants.min()
    delta_rec_sphere = time_delay_rec_sphere - time_delay_rec_sphere.min()
    chi2_sphere = np.sum((delta_exp - delta_rec_sphere)**2/(sigma**2*(nants-3)))
    return chi2_sphere
...
def ADF_3D_parameters(params, Aants, Xants, Xmax, asym_coeff=0.01):
    ...
    return(eta_array, omega_array, omega_cr_array, omega_cr_analytic_array, omega_cr_analytic_effectif_array)    
