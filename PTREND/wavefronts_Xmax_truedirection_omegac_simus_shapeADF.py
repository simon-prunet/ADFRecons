...
def ADF_3D_parameters_before_after_Xmax(params, Aants, Xants, Xmax, omega_cerenkov_array, asym_coeff=0.01):
    ...
    for i in range(nants):
        ...
        # Distribution width. Here rescaled by ratio of cosines (why ?)
        width_inf = ct / (dX[2]/l_ant) * delta_omega_inf
        width_sup = ct / (dX[2]/l_ant) * delta_omega_sup
        # Distribution
        if omega > omega_cr:
            adf = amplitude/l_ant/(1.+4.*( ((np.tan(omega)/np.tan(omega_cr))**2 - 1. )/width_sup)**2)
        else: 
            adf = amplitude/l_ant/(1.+4.*( ((np.tan(omega)/np.tan(omega_cr))**2 - 1. )/width_inf)**2)
        adf *= 1. + asym*np.cos(eta) # 
        # Chi2
        res[i]= (Aants[i]-adf)
        eta_array[i] = eta
    ...
    return(eta_array, omega_array, omega_cerenkov_simu, omega_cr_analytic_array, omega_cr_analytic_effectif_array)  

#@njit(**kwd, nopython=True)
def ADF_3D_loss_before_after_Xmax(params, Aants, Xants, Xmax, omega_cerenkov_array, asym_coeff=0.01, verbose=False):
    ...
    for i in range(nants):
        ...
        bins_etas = pd.cut(eta, bins, labels=np.array([180, -135, -90, -45, 0, 45, 90, 135, 180]), ordered=False)
        #find the index corresponding to eta_array, and find the associated omega_cr value
        eta_array = np.array([-135, -90, -45, 0, 45, 90, 135, 180])
        index = np.where(eta_array == bins_etas[0])
        index = np.int64(index[0][0])  
        omega_cr = np.deg2rad(omega_cerenkov_array[index])
        #omega_cr = (np.take(omega_cerenkov_array, index))
        #omega_cr = np.deg2rad(omega_cerenkov_array[index.item()])
...
#@njit(**kwd,nopython=True)
def ADF_3D_model_before_after_Xmax(params, Xants, Xmax, omega_cerenkov_array, asym_coeff=0.01):
