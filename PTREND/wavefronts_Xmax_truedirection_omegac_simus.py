...
#@njit(**kwd, nopython=True)
def ADF_3D_loss_new(params, Aants, Xants, Xmax, omega_cerenkov_array, asym_coeff=0.01, verbose=False):
    ...
    theta, phi, delta_omega_early, delta_omega_late, amplitude = params
    nants = Xants.shape[0]
    ct = np.cos(theta); st = np.sin(theta); cp = np.cos(phi); sp = np.sin(phi)
    # Define shower basis vectors
    K = np.array([st*cp,st*sp,ct])
    ...
    for i in range(nants):
        ...
        # Distribution width. Here rescaled by ratio of cosines (why ?)
        width_early = ct / (dX[2]/l_ant) * delta_omega_early
        width_late = ct / (dX[2]/l_ant) * delta_omega_late
        # Distribution
        #if 
        adf = amplitude/l_ant / (1.+4.*( ((np.tan(omega)/np.tan(omega_cr))**2 - 1. )/width )**2)
        adf *= 1. + asym*np.cos(eta) # 
